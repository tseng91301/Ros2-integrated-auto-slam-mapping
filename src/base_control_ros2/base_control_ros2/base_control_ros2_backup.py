...
class BaseControl(Node):
    def __init__(self):
        ...
        self.odom_timer = self.create_timer(1.0/100,self.odom_timerCB)
        ...
        self.imu_timer = self.create_timer(1.0/90,self.imu_timerCB)
                ...
                if length > 1 :
                    if self.Circleloop.get_front_second() <= self.Circleloop.get_queue_length():
                        ...
                        if(databuf[3] == 0x04):
                            ...
                            if 0 <= databuf[4]-1 < len(self.movebase_type) and 0 <= databuf[5]-1 < len(self.motor_type):
                                fRatio = float(databuf[6]<<8 | databuf[7]) / 10
                                fDiameter = float(databuf[8]<<8 | databuf[9]) / 10
                                info_string = "Type:%s Motor:%s Ratio:%.01f WheelDiameter:%.01f" % (
                                    self.movebase_type[databuf[4]-1],
                                    self.motor_type[databuf[5]-1],
                                    fRatio,
                                    fDiameter
                                    )
                                self.get_logger().info(info_string)
                                ...
                                self.get_logger().warn(
                                    f"Invalid index: movebase_type[{databuf[4]-1}] or motor_type[{databuf[5]-1}]; skipping frame"
                                    )
